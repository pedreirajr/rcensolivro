...
poa_ibge <- lookup_muni(name_muni = "Porto Alegre", year = 2022)$code_muni
...
Lembre-se de que é possível também produzir contagens para malhas espaciais além da H3 com a `cnefe_counts()`. No exemplo abaixo, utilizamos os bairros de Porto Alegre como unidades de contagem. Primeiro baixamos os bairros com `read_neighborhood()` do pacote `geobr`, restringindo o download ao município pelo argumento `code_muni`:
...
poa_brr <- read_neighborhood(year = 2022, code_muni = poa_ibge,
                             simplified = F)
```
