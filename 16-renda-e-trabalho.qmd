...
#| message: false
#| warning: false
## Lendo o arquivo ResponsavelRenda, só dos setores urbanos:
...
```{r}
#| label: coef-variacao
## Desvio padrão e coeficiente de variação da renda, por setor:
...
#| message: false
#| warning: false
## Descartando os hexágonos sem renda interpolada:
renda_h3_ocupados <- renda_h3 |>
  filter(!is.na(avg_inc_resp))

## Percentil 95, p/ limitar a escala de cor:
...
As áreas de ponderação, porém, são polígonos que cobrem o município inteiro, inclusive a parte rural, e uma delas sozinha responde por 96% da área de Manaus. Dedicar quase todo o mapa a ela deixaria o restante ilegível, e por isso reutilizamos aqui o recurso aplicado na @sec-san-banheiro-amostra do Capítulo 14, ficando apenas com as áreas de ponderação **centradas** na mancha urbana. O teste é o mesmo de lá, feito em dois passos, unindo os setores urbanos de `setores_geo` numa geometria só e verificando se o centroide de cada área de ponderação cai dentro dela.
...
Das 33 áreas de ponderação de Manaus, 32 passam no teste. A única descartada é a maior de todas, com 11.038 km², e o descarte vale apenas para os mapas, já que as estimativas municipais continuam calculadas sobre a amostra inteira. O caso de Manaus é mais confortável que o de Macapá, onde quatro das dezenove áreas ficaram de fora e três delas tinham parte relevante da sua superfície sobre a mancha urbana. Aqui a única área descartada é praticamente toda rural, de modo que o mapa não deixa de fora nenhuma porção urbana com peso.

Vale lembrar da observação feita no Capítulo 14 sobre a escolha do predicado. Manter as áreas que **cruzam** a mancha urbana, com `st_intersects()`, não descartaria nada em Manaus, pelo mesmo motivo que não descartaria nada em Macapá, já que as áreas rurais fazem fronteira com as urbanas e o simples encostar conta como interseção.
...
#| fig-height: 2.6
#| message: false
## Estimando média e mediana municipais e empilhando os resultados:
renda_mun <- bind_rows(
  pa_manaus |>
...
## Gráfico de barras com as barras de erro do IC 95%:
...
```{r}
#| label: ocupados
## Marcando quem estava ocupado e quem produzia apenas para autoconsumo:
md_manaus <- md_manaus |>
  mutate(
    ocupado = coalesce(
...
## Conferindo os totais e as ausências nas variáveis de trabalho:
md_manaus |>
  summarize(
    registros           = n(),
...
#| message: false
#| warning: false
## Estimando a proporção sem carteira assinada, por área de ponderação:
