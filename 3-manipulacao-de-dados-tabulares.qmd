...
Foi essa distinção que motivou o princípio *tidy*^[também presente no `tidyverse`] mencionado no início do capítulo, segundo o qual cada variável deve ocupar uma coluna e cada observação deve ocupar uma linha. A tabela acima viola esse princípio, e a consequência prática é que operações simples ficam trabalhosas. Calcular o total de viagens por município exige somar quatro colunas nomeadas uma a uma, e a inclusão de um quinto modo na pesquisa obrigaria a reescrever esse código.

A função `pivot_longer()`, do pacote `tidyr`, reorganiza a tabela para o formato *long*. Geralmente, são necesários informar 3 aspectos:

- Quais colunas devem ser empilhadas (`cols`).
- Qual será o nome da nova coluna que receberá os nomes antigos (`names_to`), que corresponderão aos níveis dessa nova variável.
- Qual será o nome da nova coluna que receberá os valores (`values_to`) que existiam na tabela original.
...
viagens_long <- viagens |>
  pivot_longer(
    cols      = c(a_pe, bicicleta, carro), # poderia ser somente cols = -municipio
...
Nenhum desses dois códigos menciona os nomes dos modos. Se a pesquisa passasse a registrar viagens de motocicleta, bastaria acrescentar as linhas correspondentes e ambos continuariam funcionando sem qualquer alteração. Essa independência em relação aos valores das categorias é a principal razão pela qual o formato *long* domina o trabalho de análise no R. É também o formato esperado pelo `ggplot2` para a elaboração de gráficos, o que veremos em detalhe no próximo capítulo.
...
<!-- Acho que a informação abaixo é desnecessária para um capítulo introdutório:
...
2. Crie uma nova variável na base `vel` chamada `cat_vel` que classifique as velocidades em três níveis usando `case_when()`:
   ...
   - `"Normal"`: velocidade entre 40 e 60 km/h (incluindo esses limites)
