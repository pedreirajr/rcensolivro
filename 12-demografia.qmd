...
#| message: false
#| warning: false
## Lendo o arquivo Básico, só para Goiânia:
...
#| message: false
#| warning: false
## Juntando os três arquivos pelo código do setor e renomeando as variáveis:
...
goiania_sc |>
  group_by(situacao) |>
  summarize(setores = n(), populacao = sum(pop, na.rm = TRUE)) |>
...
## Criando os indicadores das duas pontas da distribuição etária, por setor:
...
Levar os dois indicadores ao mapa mostra que eles descrevem um padrão bem marcado. Como partilham a mesma unidade, a porcentagem da população do setor, podemos exibi-los um sobre o outro com o `patchwork`.

```{r}
#| label: fig-idade-pares
#| fig-cap: "Proporção de pessoas de 0 a 4 anos (acima) e de 60 anos ou mais (abaixo) na população de cada setor censitário, Goiânia (GO), Censo 2022."
#| fig-width: 6
#| fig-height: 8
#| message: false
#| warning: false
...
## Razão entre crianças de 0 a 4 anos e pessoas idosas, por setor:
goiania_geo <- goiania_geo |>
...
#| message: false
## Somando a população de cada cor ou raça no município:
...
## Somando pretos e pardos e calculando a proporção por setor:
goiania_geo <- goiania_geo |>
  mutate(
...
#| message: false
## Agregando por subdistrito, somando cada parcela separadamente:
...
## Juntando os 5 maiores com os 5 menores em % de pretos e pardos:
bind_rows(
...
## Total de responsáveis e proporção de mulheres, por setor:
goiania_geo <- goiania_geo |>
  mutate(
...
## Responsáveis mulheres pretas ou pardas, sobre o total de responsáveis mulheres:
goiania_geo <- goiania_geo |>
  mutate(
...
## Comparando os dois denominadores possíveis:
goiania_geo |>
  st_drop_geometry() |>
  summarize(
...
```{r}
#| label: fig-resp
#| fig-cap: "Domicílios com pessoa responsável do sexo feminino (acima) e proporção de responsáveis mulheres que são pretas ou pardas (abaixo), por setor censitário, Goiânia (GO), Censo 2022."
#| fig-width: 6
#| fig-height: 8
#| message: false
#| warning: false
...
O mapa de cima é relativamente uniforme, com a chefia feminina distribuída por toda a cidade. O de baixo reproduz de perto os mapas de idade e de cor ou raça das seções anteriores, reforçando mais um aspecto do padrão socioeconômico encontrado.
...
#| message: false
#| warning: false
## Lendo o arquivo Pessoa de 2010, com a idade ano a ano:
...
## Lendo o arquivo Responsavel de 2010, que em 2022 não existe como tal:
...
## Montando o comparativo entre os dois censos:
comparativo <- tibble(
...
#| message: false
## Somando os domicílios de cada arranjo domiciliar no município:
arranjos <- goiania_sc |>
  summarize(
...
## Arranjos monoparentais sobre o total de domicílios ocupados, por setor:
goiania_geo <- goiania_geo |>
  mutate(
...
## Lendo microdados, só das áreas de ponderação de Goiânia:
...
#| message: false
## Criando as duas definições de deficiência dentro do plano amostral:
...
## Estimando a prevalência por cada critério, com intervalo de confiança:
pa_goiania |>
  summarize(
...
#| message: false
#| warning: false
## Estimando a prevalência por área de ponderação:
def_ap <- pa_goiania |>
  group_by(code_weighting) |>
...
#| message: false
#| warning: false
## Proporção de quem nasceu no município e nunca morou fora, por área:
nat_ap <- pa_goiania |>
