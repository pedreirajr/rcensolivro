...
Essa inversão de foco vem acompanhada de uma inversão de método. Enquanto todos os demais produtos nascem de uma resposta dada por um informante, as características do entorno nascem de **observação direta**, feita por um agente do IBGE que percorre a rua a pé e registra o que vê, sem entrevistar ninguém. Não há morador para dizer se a rua tem bueiro, mas sim um agente do IBGE olhando para a sarjeta e marcando "sim" ou "não" em um aplicativo.
...
No **Censo 2000**, o levantamento contemplava apenas três itens, a identificação dos logradouros, a iluminação pública e o calçamento ou pavimentação. A caracterização era feita de forma separada para o logradouro e para o setor censitário. Na Caderneta do Setor, ao encerrar seu trabalho, o recenseador indicava existência total, existência parcial ou inexistência de cada item. Na Folha de Coleta, usada para caracterizar o logradouro após percorrer a face ou o trecho, apenas o calçamento admitia a alternativa de existência parcial [@ibge_entorno2022; @ibge_metodologia2000]. Esses resultados, porém, não chegaram ao usuário na forma de dados por setor censitário. O dicionário dos Agregados por Setor Censitário de 2000 não registra nenhuma variável de entorno^[O leitor pode confirmar isso abrindo o dicionário com `censobr::data_dictionary(year = 2000, dataset = "tracts")` e procurando por qualquer um dos três itens.]. Por isso, na prática, a série de dados começa em 2010.

No **Censo 2010**, o levantamento ganhou escopo e virou uma etapa estruturada da operação. Foram dez itens investigados durante a **pré-coleta**, operação de campo inédita que antecedeu o recenseamento e cuja finalidade principal era montar a lista de endereços de cada setor. Cerca de 22 mil agentes censitários supervisores percorreram 222.541 setores censitários urbanos, nas áreas urbanizadas dos 5.565 municípios então existentes [@ibge_entorno2010]. A restrição decisiva daquela edição foi metodológica. Como a coleta exigia percorrer faces de quadra, foram excluídos da pré-coleta os setores sem arruamento regular, o que deixou de fora tanto os setores rurais quanto os aglomerados subnormais sem quadras e faces identificáveis. Esses últimos foram caracterizados por um instrumento à parte, o Levantamento de Informações Territoriais (LIT), com metodologia própria e resultados que não podem ser confrontados com os do restante da cidade [@ibge_aglomerados2010].
...
A @tbl-quesitos-censos resume quais itens foram investigados em cada edição e registra também se o resultado chegou a ser divulgado por setor censitário, informação que costuma faltar aos quadros comparativos e que determina o que efetivamente dá para analisar.
...
tibble::tribble(
  ...
  "Iluminação pública",              "coletado, não divulgado", "sim",  "sim",
  "Identificação do logradouro",     "coletado, não divulgado", "sim",  "—",
  "Calçamento ou pavimentação",      "coletado, não divulgado", "sim",  "sim",
  "Calçada ou passeio",              "—",                       "sim",  "sim",
  "Rampa para cadeirante",           "—",                       "sim",  "sim",
  "Bueiro ou boca de lobo",          "—",                       "sim",  "sim",
  "Arborização",                     "—",                       "sim",  "sim",
  "Meio-fio ou guia",                "—",                       "sim",  "—",
  "Lixo acumulado nos logradouros",  "—",                       "sim",  "—",
  "Esgoto a céu aberto",             "—",                       "sim",  "—",
  "Capacidade de circulação da via", "—",                       "—",    "sim",
  "Ponto de ônibus ou van",          "—",                       "—",    "sim",
  "Via sinalizada para bicicletas",  "—",                       "—",    "sim",
  "Obstáculo na calçada",            "—",                       "—",    "sim"
) |>
...
ggplot() +
  ...
  annotate("text", x = 6, y = 3.35, label = "Face percorrida",
           colour = "#b2182b", size = 3.3, fontface = "bold") +
  annotate("text", x = 6, y = 5.45, label = "Face confrontante",
           colour = "#2166ac", size = 3.3, fontface = "bold") +
  # calçadas
  annotate("text", x = 1.1, y = 2.5, label = "calçada", size = 2.6, colour = "grey45") +
  annotate("text", x = 1.1, y = 6.3, label = "calçada", size = 2.6, colour = "grey45") +
  # sentido do percurso do agente, sobre a calçada da face percorrida
  annotate("text", x = 9.15, y = 2.5, label = "percurso do agente",
           size = 2.6, colour = "grey30") +
  annotate("segment", x = 10.55, xend = 11.7, y = 2.5, yend = 2.5,
...
Quando este capítulo disser que determinado percentual de domicílios está em faces com calçada, o denominador serão sempre os domicílios **do universo da pesquisa**, e não todos os domicílios do município, do estado ou do país. Como a parcela excluída é justamente a de menor densidade urbana, tratar esses valores como se fossem nacionais superestima sistematicamente a infraestrutura disponível. Essa ressalva vale para 2010 e para 2022, com recortes de universo diferentes em cada ano.
:::
...
Guarde este fluxograma. Quando chegarmos aos dados, veremos que as faces que sofreram salto não desaparecem das tabelas. Elas são contabilizadas em categorias específicas de não resposta, e confundi-las com falhas de coleta produz percentuais errados.
...
#### Censo 2022, três blocos paralelos {#sec-entorno-var2022}
...
Essa organização em blocos tem uma consequência prática severa. Como não há uma variável de total de domicílios com entorno pesquisado, o denominador precisa ser reconstruído somando um bloco inteiro. E **nem todos os blocos são exaustivos**. Os blocos de condição de ocupação, de abastecimento de água e de existência de banheiro e esgotamento sanitário deixam categorias de fora, de modo que somá-los produz um total menor que o correto. Já os blocos de sexo do responsável, adequação da moradia, renda per capita, grupos de idade e cor ou raça cobrem todos os domicílios ou moradores pesquisados. A escolha do bloco de referência é, portanto, uma decisão metodológica, e não um detalhe de implementação. Trataremos disso com código na parte prática.

A @tbl-estrutura contronta estruturalmente os resultados do Entorno para ambos os censos e sintetiza o que muda de um censo para o outro.
...
tibble::tribble(
  ...
  "Número de variáveis",          "1.062, em cinco arquivos",                             "105, em um arquivo",
...
Resta esclarecer uma assimetria que atravessa todo o produto: **a coleta ocorre na face, mas a divulgação é feita no setor**. Isso quer dizer que a face de quadra é a unidade de investigação, mas não é unidade de divulgação. Não existe, no conjunto de dados públicos, uma tabela que informe as características urbanísticas de uma face específica. O que se publica é a **contagem de faces por categoria dentro de cada setor censitário**, que passa a ser a maior resolução espacial disponível. Essa diferença tem uma implicação analítica que convém fixar desde já, pois o valor de um setor é uma síntese de faces que podem ser bastante heterogêneas entre si. Um setor em que 70% das faces têm calçada não é um setor uniformemente atendido. É um setor em que três de cada dez trechos de rua não têm por onde caminhar, e a leitura do mapa precisa levar isso em conta.
...
Usaremos **Fortaleza** como pano de fundo do restante do capítulo. A escolha não é arbitrária, pois a capital cearense reúne, em um mesmo território, situações que exercitam quase tudo o que foi discutido até aqui. Ela tem um dos maiores contingentes de população em favelas e comunidades urbanas entre as capitais brasileiras, apresenta um passivo de drenagem urbana bem acima da média nacional e, ao mesmo tempo, é um caso atípico e positivo em sinalização cicloviária.

O ponto de partida é a leitura dos arquivos de entorno dos dois censos com a função `read_tracts()`, já apresentada no @sec-obtendo. O argumento `dataset` recebe o valor `"Entorno"`, e o retorno é um *Dataset* do Arrow, e não um `data.frame`. Isso importa porque o arquivo de 2022 tem quase 345 mil linhas e o de 2010 tem 310 mil, com mais de mil colunas no caso de 2010. Filtrar antes de materializar evita carregar o país inteiro na memória.
...
Como registrado na @tbl-quesitos-censos, o Censo 2000 investigou três itens do entorno, mas não os divulgou nos Agregados por Setor Censitário. Por isso o `censobr` não oferece o conjunto `Entorno` para aquele ano, e a chamada retorna erro. A série utilizável começa em 2010.
:::

Antes de qualquer cálculo, o dicionário precisa estar aberto ao lado. A função `data_dictionary()` cuida disso, com a diferença de que para 2010 ela abre um PDF, cujas seções 6.22 a 6.26 descrevem os cinco arquivos de entorno, e para 2022 abre uma pasta de trabalho `.xlsx`, cuja última aba, chamada `Entorno`, traz as variáveis que nos interessam.
...
Lembre-se que até a versão do `censobr` trabalhada no nosso texto (v0.6.0), os arquivos não vem com uma coluna de geometria. Deste modo, precisamos obter os polígonos dos setores vêm do `geobr`, com `read_census_tract()`. O argumento `code_tract` aceita o código do município para devolver todos os seus setores. Usamos `simplified = FALSE` para preservar o traçado original.
...
Os 23 setores de 2010 e os 27 de 2022 que ficam sem par **não são falha de dado**. São setores que não foram selecionados para a pesquisa, e aparecerão nos mapas com a cor reservada a valores ausentes. Repare também que a malha de Fortaleza cresceu de 3.043 para 4.408 setores entre os dois censos, informação que será decisiva na @sec-entorno-comparacao.

Em 2022 existe ainda um segundo tipo de ausência, mais sutil, que não vem do `join` e sim do próprio arquivo.
...
# Função auxiliar que protege a divisão contra denominador zero ou ausente.
# Será reaproveitada adiante, quando tratarmos do Censo 2010.
...
- **`across()`** aplica a mesma operação a várias colunas de uma só vez, dentro de um `summarise()` ou de um `mutate()`. Sem ela, seria preciso escrever uma linha de `sum()` para cada uma das 70 variáveis dos blocos de faces e de domicílios.
...
A tabela revela um padrão sistemático. Em quase todos os itens, o percentual medido sobre domicílios é **maior** que o medido sobre faces, e a diferença chega a quase dez pontos no caso da calçada. A explicação é geográfica, pois as faces mais bem servidas de infraestrutura concentram mais moradias, enquanto as faces mal servidas tendem a ser trechos de baixa densidade, terrenos vazios ou vias de borda. Ao contar domicílios, damos peso maior justamente às faces onde mais gente vive.

O item de sinalização cicloviária é a única exceção, e ela é instrutiva. Ali a relação se inverte, com 7,9% das faces contra 7,2% dos domicílios, porque a sinalização para bicicletas se concentra em vias arteriais e orlas, onde há proporcionalmente menos residências. A regra prática que decorre disso pode ser resumida assim. Faces medem **oferta de infraestrutura no território**, e domicílios medem **população atendida por ela**. Nenhuma das duas é mais correta, mas responder à pergunta errada com a unidade errada é um erro comum, e a escolha precisa ser declarada explicitamente em qualquer análise.
...
O contraste é brutal. No item de bueiro, o "Não Declarado" reúne 794 domicílios, um resíduo de menos de 0,1% que corresponde a inconsistências de coleta. Nos itens de obstáculo e de rampa, ele reúne 94.526 domicílios, e esse número não é arbitrário, pois é **exatamente** a diferença entre o total de domicílios e os que estão em faces com calçada. Não se trata de informação perdida, e sim de domicílios para os quais a pergunta não fazia sentido.
...
O mapa mostra uma cidade majoritariamente calçada com bolsões bem definidos de carência. A mediana municipal é de 91,7% das faces por setor e 53,4% dos setores estão acima de 90%, mas 554 setores, ou 12,6% do total, ficam abaixo da metade. Agregando por bairro, os extremos superiores ficam na malha consolidada, com Dionísio Torres em 97,8%, Cidade 2000 em 97,6% e Centro em 97,1%, enquanto os inferiores se concentram na frente de expansão litorânea a leste e sudeste, com Sabiaguaba em 19,1%, Manuel Dias Branco em 38,5% e Praia do Futuro II em 42,6%. É essa faixa leste que aparece em tons claros na ponta direita do mapa.
...
### Um panorama em pequenos múltiplos {#sec-entorno-painel}

Mapear item por item fica repetitivo depois do terceiro. Uma alternativa é montar um painel de pequenos múltiplos, em que os indicadores compartilham a mesma escala de cor e podem ser lidos em conjunto. Para que a comparação visual funcione, todos os itens precisam apontar no mesmo sentido, e por isso usamos arborização com **ao menos uma árvore**, somando as três classes positivas, em vez do "sem árvores" do mapa anterior. Assim, em todos os painéis, mais escuro significa melhor.

Um detalhe de implementação merece atenção. A união com a geometria deve vir **antes** do `pivot_longer()`, e não depois. Na ordem inversa, os setores sem correspondência geram uma faceta espúria rotulada como ausente, em vez de aparecerem em cinza dentro de cada painel.

```{r}
#| label: fig-for-painel
#| fig-cap: "Panorama da infraestrutura do entorno em Fortaleza (Censo 2022), medida como percentual de faces de quadra por setor censitário. Escala compartilhada entre os seis painéis."
#| fig-width: 8
#| fig-height: 5.6
#| message: false
indicadores_for <- ent_for2022 |>
  transmute(
    code_tract,
    `Iluminação pública`             = perc(faces_V05412, faces_V05400),
    `Via pavimentada`                = perc(faces_V05406, faces_V05400),
    `Calçada ou passeio`             = perc(faces_V05421, faces_V05400),
    `Arborização (1+ árvore)`        = perc(faces_V05431 + faces_V05432 +
                                              faces_V05433, faces_V05400),
    `Bueiro ou boca de lobo`         = perc(faces_V05409, faces_V05400),
    `Via sinalizada para bicicletas` = perc(faces_V05418, faces_V05400)
  )

geo_for2022 |>
  select(code_tract) |>
  left_join(indicadores_for, by = "code_tract") |>
  pivot_longer(cols = -c(code_tract, geometry),
               names_to = "indicador", values_to = "valor") |>
  mutate(indicador = factor(indicador, levels = names(indicadores_for)[-1])) |>
  ggplot() +
...
::: {.callout-note}
## `pivot_longer()` e o formato longo

A função **`pivot_longer()`**, do pacote `tidyr`, converte a tabela do formato **largo** para o formato **longo**. No formato largo, cada indicador ocupa uma coluna própria, que é como o objeto `indicadores_for` foi construído. No formato longo, todos os valores passam a ocupar **uma única coluna**, e uma segunda coluna guarda o nome do indicador ao qual cada valor pertence. O argumento `cols` diz quais colunas devem ser empilhadas, e aqui usamos a notação negativa para empilhar todas menos `code_tract` e a geometria. Os argumentos `names_to` e `values_to` dão os nomes às duas colunas resultantes.

Essa conversão é o que viabiliza o `facet_wrap()`, listado entre as camadas da gramática dos gráficos na @tbl-camadas e aqui usado pela primeira vez, porque a criação de facetas exige que a variável usada para dividir os painéis exista como **uma coluna**. Com seis indicadores em seis colunas separadas, não haveria o que passar a `~ indicador`. A tabela sai de 4.408 linhas e sete colunas para 26.448 linhas e três colunas, ou seja, seis vezes mais linhas, uma para cada combinação de setor e indicador.
:::

O painel conta a história da cidade em uma imagem. Iluminação pública e pavimentação estão praticamente universalizadas, com 96,3% e 88,6% das faces. A calçada vem logo atrás, com 79,5%, e a arborização atinge metade das faces. A drenagem destoa do conjunto, presente em apenas 28,4% das faces, bem abaixo da média nacional de 40,9%, e o mapa mostra que a carência é generalizada e não se concentra em um setor da cidade. A sinalização cicloviária, com 7,9%, parece baixa em termos absolutos, mas é mais de cinco vezes a média brasileira de 1,5%, o que faz de Fortaleza um caso nacionalmente atípico nesse item. O desenho linear e contínuo que as ciclovias formam no painel revela a lógica em rede desse tipo de infraestrutura, bem distinta da distribuição difusa dos demais itens.
...
A @sec-entorno-var2010 descreveu a lógica que organiza essas 1.062 variáveis. Vamos agora convertê-la em código. A regra é que, dentro de cada bloco, os dez itens aparecem sempre na mesma ordem, e cada um ocupa `2 × número de categorias` colunas consecutivas. Adotaremos o bloco de **sexo do responsável**, do arquivo `Entorno02`, por duas razões. Ele é exaustivo, portanto serve de denominador, e é o mais econômico, com apenas duas categorias e quatro colunas por item.
...
### Validar antes de confiar {#sec-entorno-validacao}

Um mapa de códigos construído por dedução precisa ser testado, e o teste aqui é simples e poderoso. Se o bloco é realmente exaustivo e a ordem dos itens está correta, a soma das quatro colunas tem de dar **exatamente o mesmo total** nos dez itens, porque todas elas repartem o mesmo conjunto de domicílios. Se algum total destoar, o mapeamento está errado.
...
- **`ungroup()`** desfaz esse comportamento e devolve a tabela ao processamento normal. É uma boa prática encerrar sempre um `rowwise()` ou um `group_by()` com ele, pois o agrupamento fica pendurado no objeto e afeta em silêncio as operações seguintes.
...
### A armadilha do denominador zero {#sec-entorno-nan}

Em 2022, o denominador podia estar ausente, mas nunca era zero. Em 2010, ele pode ser zero, e é aí que mora um erro silencioso. O arquivo inclui setores com domicílios registrados na variável `entorno01_V001` cujas faces não foram pesquisadas, e neles a soma do bloco dá zero.
...
tibble(
  ...
  `2010` = c(
    perc(S(ent_for2010$entorno02_V386) + S(ent_for2010$entorno02_V388),
         S(ent_for2010$total_entorno)),
    perc(S(ent_for2010$entorno02_V402) + S(ent_for2010$entorno02_V404),
         S(ent_for2010$total_entorno))
  ),
  `2022` = c(
    perc(S(ent_for2022$domicilios_V05012), S(ent_for2022$domicilios_V05000)),
    perc(S(ent_for2022$domicilios_V05009), S(ent_for2022$domicilios_V05000))
  )
) |>
...
Cada figura será composta por dois gráficos independentes, um para cada ano, unidos lado a lado pelo operador `|` do `patchwork`. Há um cuidado que não pode ser negligenciado, pois os dois painéis precisam compartilhar **exatamente os mesmos limites de escala**. Se cada mapa ajustar a paleta ao próprio intervalo de valores, o mapa de 2010 ficará tão colorido quanto o de 2022 e a evolução desaparecerá da imagem. Por isso fixamos `limits = c(0, 100)` nos dois, e `plot_layout(guides = "collect")` recolhe as duas legendas idênticas em uma só.
...
::: {.callout-warning}
## Os parênteses em torno do `|` não são decorativos
...
O escurecimento geral entre os dois painéis é visível sem esforço, e o mapa acrescenta à tabela algo que ela não podia mostrar. O ganho de 16,6 pontos da média municipal não corresponde a uma frente territorial coerente, e sim a saltos localizados espalhados por toda a cidade. Agregando por bairro, o caso extremo é o Conjunto Ceará II, no vetor oeste, que passa de 9,7% para 86,9% dos domicílios, mas ganhos expressivos aparecem igualmente na orla nordeste, no miolo e nos bairros do sul. Uma média municipal que sobe pode, portanto, esconder tanto avanços muito concentrados quanto áreas estagnadas, e é essa heterogeneidade que justifica descer ao setor censitário em vez de parar no agregado.
...
Os dois painéis são quase indistinguíveis, e essa ausência de mudança é a informação. Um item universalizado produz um mapa sem gradiente, o que costuma ser sinal de que a política pública correspondente cumpriu seu objetivo e o indicador perdeu poder discriminante.

Resta a ressalva final, que é independente dos itens escolhidos. Fortaleza passou de 3.020 para 4.381 setores pesquisados entre os dois censos, e a expansão recaiu sobre áreas que antes ficavam fora do universo da pesquisa. Parte da variação observada reflete esse alargamento, e não apenas mudança real no território.

O efeito é verificável nos próprios dados. Entre os 87 bairros de Fortaleza com volume suficiente para comparação, cinco registram **queda** no percentual de domicílios com bueiro, o que é contraintuitivo para um item de infraestrutura permanente medido por critério mínimo. Os dois maiores recuos ocorrem justamente nos bairros em que o universo pesquisado mais cresceu, com o Dendê triplicando o número de domicílios pesquisados e o Benfica quase dobrando. Não se trata de bueiros que desapareceram, e sim de áreas antes não pesquisadas que passaram a compor o denominador. Quando o objetivo for medir evolução com rigor, o caminho mais seguro é examinar a variação do universo antes de interpretar a variação do indicador.
...
2. Reproduza o painel de pequenos múltiplos da @fig-for-painel para outra capital de sua escolha, mantendo os mesmos seis indicadores e a mesma escala. Compare o resultado com Fortaleza, prestando atenção especial ao item de sinalização cicloviária, e verifique se o valor encontrado se aproxima mais da média nacional de 1,5% das faces ou do caso fortalezense.

3. Calcule a taxa de rampa para cadeirante nas duas versões apresentadas na @sec-entorno-nd, sobre o total de domicílios e condicionada à existência de calçada, para os cinco maiores municípios de sua unidade da federação. Discuta qual das duas você apresentaria em um relatório de acessibilidade urbana e por quê.

4. Use o dicionário construído na @sec-entorno-r2010 para mapear, por setor censitário, o percentual de domicílios com esgoto a céu aberto no entorno em Fortaleza em 2010. Esse item não existe no Censo 2022, o que impede qualquer comparação temporal, mas não impede a análise do retrato de 2010. Compare o padrão espacial obtido com o do bueiro no mesmo ano e comente a relação entre os dois.
