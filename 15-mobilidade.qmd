...
No planejamento de transportes, **acessibilidade** designa a facilidade com que se alcança um destino a partir de um lugar, e é uma função da rede de transporte e da distribuição das atividades no território. Calculá-la nesse sentido estrito exige matrizes de tempo de viagem e a localização das oportunidades de emprego, saúde e educação, insumos que o Censo Demográfico não fornece.
...
Como os microdados de 2022 ainda não foram divulgados, conforme discutido no Capítulo 8, todo o comportamento de deslocamento deste capítulo vem de 2010. Todavia, a infraestrutura que vem do Entorno é de 2022, e a @sec-mob-acessibilidade explica por que ela não é comparada com a de 2010.

## Estruturação dos microdados da amostra de 2010 {#sec-mob-dados}

Como sabemos, os microdados da amostra são divulgados por área de ponderação, unidade apresentada na @sec-areas-pond do Capítulo 8. Precisamos, então, do código do município e da malha dessas áreas, que servirá de base para todos os mapas da primeira metade do capítulo.
...
A conduta correta, portanto, não é descartar os ausentes com `na.rm = TRUE`, e sim restringir a análise ao universo em que a pergunta foi feita. As duas coisas produzem o mesmo número neste caso, mas apenas a segunda deixa o universo explícito para quem lê. Feito o recorte, declaramos o desenho amostral na forma apresentada no Capítulo 8.
...
## Posse de veículos {#sec-mob-veiculos}

A posse de veículo não mede mobilidade, mas uma *possibilidade*, configurando, inclusive, uma medida importante de acessibilidade. Um domicílio com automóvel tem uma alternativa de deslocamento que um domicílio sem automóvel não tem, e essa diferença estrutura tudo o mais, do tempo de viagem à sensibilidade à qualidade do transporte público. Comecemos pelo retrato municipal, com os intervalos de confiança que o desenho amostral exige.
...
posse_mun <- bind_rows(
  pa_dom |>
    summarise(proporcao = survey_mean(V0222 == "1", vartype = "ci")) |>
    mutate(veiculo = "Automóvel"),
  pa_dom |>
    summarise(proporcao = survey_mean(V0221 == "1", vartype = "ci")) |>
...
Fixado o universo, podemos calcular. O tempo de deslocamento é o indicador de mobilidade mais próximo da experiência cotidiana, e em 2010 ele foi coletado em cinco faixas, o que impede calcular média ou mediana e limita a análise a proporções.

Declaramos o universo filtrando o próprio desenho amostral, e não a tabela de origem. Essa distinção importa, porque o `srvyr` trata o subconjunto como domínio e calcula os erros-padrão levando em conta que ele é parte de uma amostra maior, o que um filtro aplicado antes de `as_survey_design()` não faria. A @tbl-mob-tempo traz a distribuição pelas cinco faixas.
...
Cerca de quatro em cada dez viagens cabiam em meia hora e **pouco mais de um quinto passava de uma hora**. Esse último grupo é o que interessa à política pública, porque tempo de deslocamento excessivo é uma forma de privação que se acumula todos os dias e que consome horas que não voltam. Vale registrar que essa proporção é bem superior à que se observa na maior parte dos municípios brasileiros, e ela dá a medida do que significa atravessar Salvador em hora de pico.
...
Convém ter em mente, desde já, que **a área observada não é a mesma em todos os itens**, conforme a @tbl-criterios do Capítulo 11 detalha. Os itens ligados à via, entre eles a pavimentação, o ponto de ônibus e a sinalização para bicicleta, são observados na face percorrida, na face confrontante e no canteiro central. Já os itens ligados à calçada, que são a existência, o obstáculo e a rampa, são observados **somente na face percorrida**. Percentuais desses dois grupos apresentados lado a lado não descrevem a mesma extensão de espaço público.
...
Levemos os indicadores ao mapa. A leitura das geometrias exige dois cuidados, ambos resolvidos no mesmo bloco de código. O primeiro é descartar o setor `292740805100008`, que tem 386 km², nenhum morador e nenhuma classificação de situação, porque corresponde à porção da Baía de Todos-os-Santos que está dentro dos limites municipais. Mantê-lo comprimiria a cidade em um canto da figura, como aconteceu com a área de ponderação da @fig-veiculos. O filtro `!is.na(situacao)` dá conta dele, e as ilhas habitadas permanecem no mapa. O segundo é que a malha do `geobr` traz três setores repartidos em mais de uma linha, por terem partes desconexas, e é preciso dissolvê-los pelo código com `group_by()` e `summarize()`, na forma apresentada no Capítulo 5, para que nenhum setor entre duas vezes nas somas por bairro.
